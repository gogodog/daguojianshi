...
public class DraftServiceImpl implements DraftService{
	...
	public Draft selectByIdAll(String id) {
		Draft draft = draftMapper.selectByIdAll(id);
		if(draft!=null){
	...
	public int publish(String id, Integer publish_user_id, Long visits, Date showTime, boolean isShowNow)
		...
		int flag = 0;
		Date now = new Date();
		Draft draft = draftMapper.publish(id, publish_user_id, now);
		if(draft!=null){
			Articlescrap articlescrap = Draft.transToArticlescrap(draft, visits, showTime);
			//如果需要立刻展示
			if(isShowNow){
	...
	public PageInfoDto<DraftDto> listDraftsWithAPRecord(DraftCondition condition) {
		PageInfoDto<Draft> pageinfo = listDrafts(condition);
